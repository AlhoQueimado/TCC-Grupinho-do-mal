<?php
  ...
  if ($_GET['id']){
    ...
    $sql = "SELECT id_usuario, login_usuario, senha_usuario, matricula_usuario, email_usuario, imagem_usuario FROM usuario WHERE id_usuario =\"$_GET[id]\"";
...
<form class="formulario" action="registrarUsuario.php" method="post" enctype="multipart/form-data">
  ...
  <input type="number" name="matriculaUsuario" value="<?php echo $usuario[matricula_usuario] ?>" placeholder="Matrícula" ><br><br>
  Foto:<br> <input type="file" name="imagemUsuario" ><br ><br >
  <?php if ($_GET['id']){

    echo "<img src=\"$usuario[imagem_usuario]\" width=\"125\" height=\"125\"><br ><br >";
    $sql= "SELECT * FROM agrupamento";
    $resultado = mysqli_query($conexao, $sql);
   ?>
  Função: <br ><br ><select name="funcaoUsuario"> <?php
